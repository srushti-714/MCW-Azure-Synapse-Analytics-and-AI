...
$uniqueId = Read-Host -Prompt "Enter the unique suffix you used in the deployment"#>
        ...
        $resourceGroupName = (Get-AzResourceGroup | Where-Object { $_.ResourceGroupName -like "*Synapse-MCW-*" }).ResourceGroupName
...
$destination = $dataLakeStorageUrl +"wwi-02/product-json" + $destinationSasKey
  & $azCopyCommand copy $rawData $destination --recursive=true
